...
| Supervised Org Plan execution inserted a coordination-only supervisor between the root reviewer and each L1 executor. | The root now combines director, reviewer, and supervisor duties and directly manages one fresh depth-one executor per L1. The Luna supervisor profile and depth-two requirement were removed. |
...
| Context-mode used a custom hook manifest path and lacked install-surface fields required by the generic plugin validator. | Its Codex hooks now occupy the auto-discovered `hooks/hooks.json` path, the redundant manifest override is gone, and its interface metadata is complete. |
