...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'wait for its explicit ACCEPT or REJECT verdict.'
...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'Filter executor output into upstream decisions, actionable findings, commit IDs or ranges, test commands with pass/fail summaries, dirty-scope results, and blockers'
...
expect_contains "$skill" 'UNREVIEWED and the supervisor returns corrections to the same L1 executor'
