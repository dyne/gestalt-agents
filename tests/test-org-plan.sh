...
expect_contains "$skill" 'The supervisor spawns the executor with'
expect_contains "$skill" 'reviewer audit the same way.'
expect_contains "$skill" 'The supervisor keeps only one write-capable child active.'
expect_contains "$skill" 'executor to finish tests and commit before starting the reviewer.'
expect_contains "$skill" 'follow-up assignments to the same executor'
...
expect_contains "$skill" 'shared-code regression impact, and named evidence.'
...
expect_contains "$skill" 'supervisor returns the corrections to the executor'
...
expect_contains "$skill" 'required tests, the exactly-one-commit rule, preserved paths'
...
expect_contains "$skill" 'Every role receives its checklist as a complete fresh assignment.'
expect_contains "$skill" 'parent-context references such as "continue above"'
