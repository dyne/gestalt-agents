...
expect_contains "$supervised" 'director/reviewer/supervisor (depth 0, read-only root)'
expect_contains "$supervised" 'executor (depth 1, only code writer)'
expect_contains "$supervised" 'fork_turns=none'
expect_contains "$supervised" 'Codex must permit `[agents] max_depth = 1`.'
expect_contains "$supervised" 'The root owns user communication, directly launches each executor'
...
expect_contains "$readme" 'director/reviewer/supervisor (depth 0, org-plan-reviewer, Sol or Terra, read-only)'
