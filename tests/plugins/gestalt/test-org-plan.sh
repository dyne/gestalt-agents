...
expect_contains "$profile" 'report only to the root director/reviewer'
...
expect_contains "$encoded_dir/encoded-executor.toml" 'verify every reference in the assigned L1 Skills list is available'
...
expect_contains "$encoded_dir/encoded-executor.toml" 'The baseline never needs to appear in the Org plan Skills list.'
...
expect_contains "$encoded_dir/encoded-executor.toml" 'Whenever an L2 reaches DONE, stop that turn and return a concise structured report'
expect_contains "$encoded_dir/encoded-executor.toml" 'Never commit at an L2 boundary.'
...
expect_contains "$supervision_dir/org-plan-executor.toml" 'Remain assigned to the whole L1 and resume only when the root follows up.'
...
expect_contains "$supervision_dir/org-plan-executor.toml" 'report only to the root director/reviewer'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Report boundaries are root-only presentation boundaries, never review authority or progress approval.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Completion; Files; Verification; Commit: Pending L<a> acceptance; Next.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Without l2Completed support, summarize in commentary and use same-turn follow-up.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Pass exact literal task_name l<a> for position L<a>: l1, then l2, and so on.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Status prose and waiting statements are never dispositions.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'call gestalt_autopilot_wait_lease version 2'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Yield only when its response contains accepted:true.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'an unavailable tool or accepted:false requires same-turn supervision.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'reason wakeAlreadySatisfied means the executor event is already actionable'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'resumes on the first matching event or deadline and changes no permanent pulse policy'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'gh pr checks <PR> --watch --interval 30'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'lease processExited and processResultAvailable'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'only a physically unavailable slot permits l<a>_gN.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'never increment a generation speculatively'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'displayed roster identity is exactly root `l0` and executor `l<a>`'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Your exact roster name is l0.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'never append a title, role, nickname, plan name, L2 position, or generated display label.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'with `L<a>/TOTAL — TITLE: STATUS`'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Supervision is a completion loop with finite reporting boundaries.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'After every executor report, inspect the executor current state.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'After an L2 final, the next Autopilot root turn follows up the same executor'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'For each validated DONE L2, call gestalt_org_plan_checkpoint with l2Completed when available'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'as the last tool call of the root turn'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Make no intervening tool call, do not call followup_task, and do not begin L1 review before that final'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Even the last L2 gets its own boundary before later L1 review.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'spawn one fresh depth-one subagent with fork_turns=none, agent_type=org-plan-reviewer, and task_name=final_review'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'dedicated role owns reviewer model selection'
...
expect_contains "$skill" '`agent_type=org-plan-reviewer`, and `task_name=final_review`'
expect_contains "$skill" 'dedicated role fixes the reviewer model to Sol'
...
expect_contains "$skill" 'Execution is completion-driven with explicit report boundaries.'
expect_contains "$skill" '`gestalt_org_plan_checkpoint` once with `l2Completed`'
expect_contains "$skill" 'short, idempotent persist-and-ack boundary'
expect_contains "$skill" '`checkpointChanged`'
expect_contains "$skill" '`checkpointHandoffFailed`'
expect_contains "$skill" '`safetyPaused`'
expect_contains "$skill" 'Never increment a generation'
expect_contains "$skill" 'legacy same-turn fallback'
expect_contains "$skill" 'Yield only when the tool response contains `accepted:true`.'
expect_contains "$skill" 'unavailable or returns `accepted:false`'
expect_contains "$skill" '`reason:"wakeAlreadySatisfied"` means the'
expect_contains "$skill" 'Do not retry the lease.'
...
expect_contains "$supervised" 'following root turn, call `followup_task` on that same executor.'
...
expect_contains "$supervised" 'A partial report, idle executor, self-described pause, or token or'
...
expect_contains "$supervised" '`agent_type=org-plan-reviewer`'
expect_contains "$supervised" 'role fixes the reviewer model to Sol'
...
expect_contains "$supervised" '## Completed-L2 reporting boundary'
expect_contains "$supervised" 'L<a>.<b>/CHILD_TOTAL — TITLE: DONE'
expect_contains "$supervised" 'Commit: Pending L<a> acceptance; changes remain uncommitted.'
...
expect_contains "$supervised" 'Never increment a generation speculatively'
expect_contains "$supervised" 'short, idempotent persist-and-ack boundary'
expect_contains "$supervised" '`checkpointChanged`'
...
expect_contains "$readme" 'the root validates and publishes that L2 as a compact final'
expect_contains "$readme" 'the next automatic turn resumes the same executor.'
expect_contains "$readme" 'Accepted L1 answers roll up their L2s and name the resulting'
...
expect_contains "$agents" 'root is `l0`; the executor for L1 position `a` is exactly `l<a>`.'
expect_contains "$agents" 'literal value as `task_name` (`l1`, then `l2`, and so on).'
expect_contains "$agents" 'increment a generation speculatively'
expect_contains "$agents" 'After either boundary checkpoint, make no more tool calls, launch no'
...
expect_contains "$agents" 'Treat supervision as a completion loop with explicit report boundaries.'
...
expect_contains "$agents" 'executor owns its whole assigned L1'
expect_contains "$agents" 'The root validates that state and evidence,'
expect_contains "$agents" 'starts the next root turn, where the root resumes the same executor'
expect_contains "$agents" 'short, idempotent persist-and-ack boundary'
expect_contains "$agents" '`checkpointChanged`'
expect_contains "$agents" '`checkpointHandoffFailed`'
expect_contains "$agents" '`safetyPaused`'
...
expect_contains "$agents" 'exactly one concise root final with Completion, Files, Verification, Commit,'
...
expect_contains "$agents" '`agent_type=org-plan-reviewer`'
