...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'exact roster identity is root `l0` and executor `l<a> — <current L1 title>`'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Your exact roster name is l0.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Never invent role, nickname, plan-title, or generated display names.'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Do not call followup_task before that final'
...
expect_contains "$agents" 'root is `l0`; the executor for L1 position `a` is `l<a> — <L1 title>`'
