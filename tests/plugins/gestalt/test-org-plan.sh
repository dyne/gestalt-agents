...
expect_contains "$encoded_dir/encoded-executor.toml" 'load the mandatory $context-mode:context-mode baseline'
...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'Before any work, load the mandatory $context-mode:context-mode baseline'
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'verify and load every declared task-specific skill, and load no other optional or task skill'
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'an unavailable baseline or declared reference blocks the L1 without edits.'
...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'the director renders the verdict from the supplied evidence and the supervisor continues without asking the user for progress or review approval.'
...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'Run potentially large repository inspections, test commands, and log processing with context-mode'
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'so raw output does not enter conversational context.'
...
expect_contains "$supervision_dir/org-plan-supervisor.toml" 'Do not install or enable context-mode automatically when unavailable.'
...
expect_contains "$supervision_dir/org-plan-executor.toml" 'load the mandatory $context-mode:context-mode baseline'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Before any work, load the mandatory $context-mode:context-mode baseline'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Render routine review verdicts autonomously from the supplied evidence; never ask the user for progress or review approval.'
...
expect_contains "$skill" 'Record exact task-specific references in `:SKILLS:`'
expect_contains "$skill" 'The executor'
expect_contains "$skill" 'loads `$context-mode:context-mode` plus exactly the declared L1 skills and no'
expect_contains "$skill" 'Do not include it in `:SKILLS:`; reserve that property for'
...
expect_contains "$skill" 'repository inspection or implementation, the executor loads the mandatory'
expect_contains "$skill" 'loads every listed skill, and loads no other optional or task'
expect_contains "$skill" 'An unavailable baseline or declared reference blocks the L1 without edits.'
...
expect_contains "$skill" 'Every role must load'
expect_contains "$skill" 'no role installs or'
expect_contains "$skill" 'enables it automatically when unavailable.'
...
expect_contains "$skill" 'the exact `:SKILLS:` list, the mandatory `$context-mode:context-mode` baseline,'
expect_contains "$skill" 'and the declared-task-skills-only gate'
...
expect_contains "$skill" 'neither role asks the user for a progress decision or review approval.'
...
expect_contains "$skill" 'load the mandatory `$context-mode:context-mode` baseline, verify every declared'
expect_contains "$skill" 'then load exactly those task-specific skills and no'
expect_contains "$skill" 'optional or task skill before studying the L1 or repository.'
expect_contains "$skill" 'Stop only for an unavailable context-mode baseline, unavailable declared skill,'
...
expect_contains "$readme" 'For every Org Plan, each active Codex role loads `$context-mode:context-mode` as'
expect_contains "$readme" 'list `$context-mode:context-mode`; it is an implicit baseline for every role.'
expect_contains "$readme" 'fresh executor loads that baseline plus exactly the declared task-specific list'
...
expect_contains "$agents" '`$context-mode:context-mode`; every role loads it as a mandatory baseline.'
...
expect_contains "$agents" '`$context-mode:context-mode` skill in every role, but do not install or enable'
