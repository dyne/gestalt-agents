...
expect_contains "$skill" '`$context-mode:context-mode` is an implicit execution baseline.'
...
expect_contains "$supervised" 'director/reviewer (depth 0, read-only)'
expect_contains "$supervised" 'supervisor (depth 1, coordination only)'
expect_contains "$supervised" 'executor (depth 2, only code writer)'
expect_contains "$supervised" 'fork_turns=none'
expect_contains "$supervised" 'Codex must permit `[agents] max_depth = 2`.'
expect_contains "$supervised" 'The supervisor launches exactly one fresh executor per L1'
expect_contains "$supervised" 'The director returns structured findings plus explicit ACCEPT or REJECT.'
expect_contains "$supervised" 'Never relay raw logs or complete child transcripts upward.'
expect_contains "$supervised" 'L1 POSITION/TOTAL — TITLE: STATUS'
