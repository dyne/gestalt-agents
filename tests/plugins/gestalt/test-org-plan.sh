...
expect_contains "$profile" 'You are the depth-one executor and only code writer.'
expect_contains "$profile" 'report only to the root.'
...
expect_contains "$supervision_dir/org-plan-executor.toml" 'Never include an Org Plan in Git or relay raw logs.'
expect_not_contains "$supervision_dir/org-plan-executor.toml" 'At the start of each new or resumed relay session'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'A retained same-session signal preserves explicit manual Off'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Use task_name l<a>; use l<a>_gN only after the prior physical slot is confirmed unavailable'
...
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'make l2Completed the last tool call'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'End the root turn immediately after either final.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Without checkpoint support, continue in the same turn.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Every DONE L2 and accepted L1 ends with one finite answer'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'starts in a new root turn and answer.'
...
python3 -c 'import pathlib, sys; words=len(pathlib.Path(sys.argv[1]).read_text().split()); assert words <= 850, words' "$supervision_dir/org-plan-reviewer.toml" && pass || fail 'reviewer profile stays within its prompt budget'
python3 -c 'import pathlib, sys; words=len(pathlib.Path(sys.argv[1]).read_text().split()); assert words <= 450, words' "$supervision_dir/org-plan-executor.toml" && pass || fail 'executor profile stays within its prompt budget'
...
expect_contains "$skill" 'Yield only after `accepted:true`.'
expect_contains "$skill" '`accepted:false`, or `wakeAlreadySatisfied`'
expect_contains "$skill" 'A lease covers one episode; it never waits for a'
expect_contains "$skill" 'Authoring never authorizes execution:'
expect_contains "$skill" 'after the validated handoff, stop until'
expect_contains "$skill" 'later work starts in a new root turn.'
...
python3 -c 'import pathlib, sys; words=len(pathlib.Path(sys.argv[1]).read_text().split()); assert words <= 2450, words' "$skill" && pass || fail 'org-plan skill stays within its prompt budget'
expect_not_contains "$skill" 'root session'
expect_not_contains "$readme" 'root session'
...
expect_contains "$supervised" 'root (depth 0, org-plan-reviewer, Sol or Terra, read-only)'
...
expect_contains "$supervised" 'The root owns director, supervisor, and routine reviewer duties.'
...
python3 -c 'import pathlib, sys; words=len(pathlib.Path(sys.argv[1]).read_text().split()); assert words <= 3350, words' "$supervised" && pass || fail 'supervised reference stays within its prompt budget'
expect_not_contains "$supervised" 'root session'
expect_not_contains "$supervised" 'presentation boundary'
expect_not_contains "$supervised" 'reporting boundary'
...
expect_contains "$supervised" '## Accepted-L1 boundary'
expect_contains "$supervised" '## Completed-L2 boundary'
...
expect_contains "$skill" 'At the beginning of every new or resumed relay session'
expect_contains "$supervised" 'before inspecting milestone state,'
expect_contains "$agents" 'At the start of each relay session supervising an incomplete plan'
expect_contains "$readme" 'Every new or resumed relay session signals supervision'
...
expect_contains "$readme" 'root (depth 0, org-plan-reviewer, Sol or Terra, read-only)'
...
expect_contains "$agents" '## Org Plan development'
expect_contains "$agents" 'The canonical runtime contract is'
expect_contains "$agents" 'Generated profiles in `scripts/org-plan` are'
expect_contains "$agents" 'Use the canonical terms `root`, `executor`, `terminal reviewer`, `relay'
expect_contains "$agents" 'At the start of each relay session supervising an incomplete plan'
expect_contains "$agents" 'Treat executor completion, error, interruption, and idle as wake inputs.'
expect_contains "$agents" 'A wait lease permits yielding only after'
expect_contains "$agents" 'A checkpoint is the last tool call of its root turn.'
expect_contains "$agents" 'A checkpoint is never a wait'
expect_contains "$agents" 'Org Plan files are workspace-local coordination state and never Git'
expect_contains "$agents" '`git diff --cached --name-only`'
expect_contains "$agents" 'Use `$gestalt:context-mode` for large or uncertain'
expect_contains "$agents" 'startup ordering, parseability, and word budgets'
python3 -c 'import pathlib, sys; words=len(pathlib.Path(sys.argv[1]).read_text().split()); assert words <= 900, words' "$agents" && pass || fail 'AGENTS guidance stays within its prompt budget'
...
expect_contains "$agents" 'run `prepare-supervision` during ordinary execution.'
expect_contains "$cli_state" '`org-plan signal PLAN resync`'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'org-plan signal PLAN supervision-start'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'First tool call: read the org-plan skill completely and do nothing else.'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Use org-plan commands for every lifecycle transition and measurement'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'the root projects Org state through host update_plan'
expect_contains "$supervision_dir/org-plan-reviewer.toml" 'Org state remains authoritative if projection fails.'
expect_contains "$supervision_dir/org-plan-executor.toml" 'Use scripts/org-plan for lifecycle changes and measurement.'
expect_contains "$supervision_dir/org-plan-executor.toml" 'Executors report mutations but never project.'
expect_contains "$encoded_dir/encoded-executor.toml" 'Use scripts/org-plan for lifecycle changes and measurement.'
