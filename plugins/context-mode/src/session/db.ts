  ...
  if (envSuffix !== undefined) {
    ...
    try {
      ...
      if (currentRoot && mainRoot) {
        ...
        // passed in.
        const canonicalCurrent = canonicalizeProjectRoot(currentRoot);
        const canonicalMain = canonicalizeProjectRoot(mainRoot);
...
export function hashProjectDirCanonical(projectDir: string): string {
  return createHash("sha256").update(canonicalizeProjectRoot(projectDir)).digest("hex").slice(0, 16);
}
