...
export function resolveCodexSessionCwd(opts?: {
  ...
  now?: number;
  /**
   * The Codex child-process thread id. When available, only a transcript whose
   * filename belongs to this thread may authorize the project root.
   */
  threadId?: string;
}): string | null {
  const codexHome =
  ...
  const visit = (dir: string, depth: number) => {
    ...
    for (const entry of entries) {
      ...
      if (opts?.threadId && !entry.endsWith(`${opts.threadId}.jsonl`)) continue;
...
export function resolveProjectDir(opts: {
   ...
   */
  codexHome?: string;
  /** Codex request/thread identity; prevents a sibling session transcript winning by mtime. */
  codexThreadId?: string;
}): string {
  const {
    env, cwd, pwd, transcriptsRoot, transcriptMaxAgeMs, nowMs, strictPlatform, codexHome, codexThreadId,
  } = opts;
