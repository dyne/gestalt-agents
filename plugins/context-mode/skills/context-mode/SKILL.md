...
└── Reading a file to analyze/summarize (not edit)?
    ├── Is it a selected `SKILL.md`, governing `AGENTS.md`, or required plugin manifest?
    │   └── Use Codex's native owning-environment read path; these control-plane instructions are host-authorized.
    └── Otherwise use ctx_execute_file (file loads into FILE_CONTENT, not context)
```
...
9. **Keep control-plane reads native.** In Codex, load selected `SKILL.md`, governing `AGENTS.md`, and activation manifests through the native reader. Use context-mode only for the data-plane analysis those instructions request.
