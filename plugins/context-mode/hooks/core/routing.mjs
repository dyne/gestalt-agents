 ...
 *
 * Loud fail if the prepared security bundle is unavailable. Without this,
 * user-configured `permissions.deny` patterns
 ...
 */
export async function initSecurity(_buildDir) {
  ...
  if (!securityInitFailed && !process.env.CONTEXT_MODE_SUPPRESS_SECURITY_WARNING) {
    process.stderr.write(
      `[context-mode] WARNING: security module not found — security deny patterns will NOT be enforced.\n` +
        `  Missing: ${bundlePath}. Run gestalt-setup.sh, then restart Codex.\n` +
...
export function buildSecurityWarningContext() {
  ...
  return [
    ...
    "  </issue>",
    "  <root_cause>",
    "    `hooks/security.bundle.mjs` is absent or unloadable.",
    "  </root_cause>",
    "  <fix>",
    "    Run `gestalt-setup.sh`, then restart Codex.",
    "    To opt in to fail-CLOSED instead, set CONTEXT_MODE_REQUIRE_SECURITY=1.",
...
export function routePreToolUse(toolName, toolInput, projectDir, platform, sessionId, options = {}) {
  ...
  if (canonical === "Agent") {
    ...
    const subagentBlock = platform === "codex"
      ? createCodexRoutingReminder(t)
      : createRoutingBlock(t, {
