...
describe("getProjectDir() under Codex platform detection (issue #45)", () => {
  ...
  it("recovers cwd from Codex session log when env is empty and Codex is the detected platform", async () => {
    ...
    expect(resolveCodexToolContext()).toEqual({
      projectDir: expectedCwd,
      sessionId: "abc-123",
    });
  });
});

describe("Codex request context isolation", () => {
  it("keeps interleaved tool contexts isolated for project and attribution", async () => {
    const first = withProjectDirOverride({ projectDir: "/project/one", sessionId: "thread-one" }, async () => {
      await new Promise((resolve) => setTimeout(resolve, 5));
      return { projectDir: getProjectDir(), attribution: currentAttribution() };
    });
    const second = withProjectDirOverride({ projectDir: "/project/two", sessionId: "thread-two" }, async () => {
      await new Promise((resolve) => setTimeout(resolve, 1));
      return { projectDir: getProjectDir(), attribution: currentAttribution() };
    });

    await expect(Promise.all([first, second])).resolves.toEqual([
      { projectDir: "/project/one", attribution: { sessionId: "thread-one" } },
      { projectDir: "/project/two", attribution: { sessionId: "thread-two" } },
    ]);
  });
});
