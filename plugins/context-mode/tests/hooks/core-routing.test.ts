...
describe("routePreToolUse", () => {
  ...
  describe("Subagent ctx_commands omission (#233)", () => {
    ...
    it("injects only compact routing guidance for Codex subagents", () => {
      const result = routePreToolUse(
        "Agent",
        { prompt: "Research this repository", subagent_type: "general-purpose" },
        undefined,
        "codex",
      );
      expect(result?.action).toBe("modify");
      ...
      expect(prompt).not.toContain("<context_window_protection>");
      expect(Buffer.byteLength(prompt, "utf8")).toBeLessThan(512);
    });
