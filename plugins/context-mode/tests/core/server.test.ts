...
describe("MCP initialization instructions", () => {
  test("Codex receives a compact, self-contained routing contract", async () => {
    const proc = startMcpServer({ CONTEXT_MODE_PLATFORM: "codex" });
    sendRpc(proc, {
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "codex", version: "1.0" } },
    });
    const [init] = await collectRpcResponses(proc, 15_000, [1]);
    const instructions = init?.result?.instructions ?? "";

    expect(instructions).toContain("ctx_batch_execute");
    expect(instructions).toContain("ctx_execute_file");
    expect(instructions).toContain("derived findings");
    expect(instructions).toContain("native edit tools");
    expect(instructions).toContain("AGENTS.md");
    expect(Buffer.byteLength(instructions, "utf8")).toBeLessThanOrEqual(512);
  }, 30_000);

  test("non-Codex clients retain compatible initialization instructions", async () => {
    const proc = startMcpServer({ CONTEXT_MODE_PLATFORM: "gemini-cli" });
    sendRpc(proc, {
      jsonrpc: "2.0", id: 1, method: "initialize",
      params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "generic-client", version: "1.0" } },
    });
    const [init] = await collectRpcResponses(proc, 15_000, [1]);
    expect(init?.result?.instructions).toContain("ctx_search");
  }, 30_000);
});
...
describe("ctx_doctor hook script checks", () => {
  ...
  test("resolves relative hook script paths against pluginRoot", () => {
    const doctorSection = serverSrc.slice(
      serverSrc.indexOf('server.registerTool(\n  "ctx_doctor"'),
      serverSrc.indexOf('server.registerTool(\n  "ctx_upgrade"'),
    );
...
describe("MCP-visible tool description budget", () => {
  const tools = REGISTERED_CTX_TOOLS as Array<{ name: string; config: { description?: string } }>;

  test("keeps the aggregate description payload within its documented budget", () => {
    const total = tools.reduce(
      (bytes, tool) => bytes + Buffer.byteLength(tool.config.description ?? "", "utf8"),
      0,
    );
    expect(total).toBeLessThanOrEqual(MCP_TOOL_DESCRIPTION_BUDGET_BYTES);
  });

  test("preserves decisive safety and routing cues", () => {
    expect(tools.find((tool) => tool.name === "ctx_purge")?.config.description).toContain("DESTRUCTIVE");
    expect(tools.find((tool) => tool.name === "ctx_execute_file")?.config.description).toContain("native Read before editing");
    expect(tools.find((tool) => tool.name === "ctx_batch_execute")?.config.description).toContain("independent I/O");
  });
});
