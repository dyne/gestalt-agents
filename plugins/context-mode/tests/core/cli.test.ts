...
describe(".mcp.json — MCP server config", () => {
  ...
  it("package.json files[] ships the canonical Codex .mcp.json manifest", () => {
    // Codex discovers this root-level companion file through
    // .codex-plugin/plugin.json and does not interpolate plugin-root
    // placeholders. Keep its relative-path contract separate from the Claude
    // template and ensure npm installs carry the manifest.
    const pkg = JSON.parse(readFileSync(resolve(ROOT, "package.json"), "utf-8"));
