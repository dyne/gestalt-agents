 ...
 * The repo ships two Claude sibling manifests plus a separate Codex companion:
 *
 *   1. `.mcp.json.example`                    (Claude contributor template)
 *   2. `.claude-plugin/plugin.json`           (Claude marketplace manifest)
 *   3. `.mcp.json`                            (Codex plugin companion)
 *
...
describe("Issue #531 — asymmetric-drift invariant", () => {
  ...
  test("Codex .mcp.json uses its literal relative entrypoint", () => {
    const got = readArgs0(resolve(ROOT, ".mcp.json"), "context-mode");
    expect(got, ".mcp.json missing or args[0] not a string").toBe(CODEX_ENTRYPOINT);
  });
