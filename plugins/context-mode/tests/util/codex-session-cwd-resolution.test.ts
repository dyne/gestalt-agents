...
describe("resolveCodexSessionCwd", () => {
  ...
  it("uses the matching CODEX_THREAD_ID transcript instead of a newer sibling session", () => {
    const codexHome = makeCodexHome();
    writeSession(codexHome, "thread-current", "/project/current", new Date(Date.now() - 60_000));
    writeSession(codexHome, "thread-sibling", "/project/sibling", new Date());

    expect(resolveCodexSessionCwd({ codexHome, threadId: "thread-current" })).toBe("/project/current");
  });

  it("does not fall back to a sibling transcript when the requested thread is absent", () => {
    const codexHome = makeCodexHome();
    writeSession(codexHome, "thread-sibling", "/project/sibling", new Date());

    expect(resolveCodexSessionCwd({ codexHome, threadId: "thread-current" })).toBeNull();
  });
