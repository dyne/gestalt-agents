...
describe("hashProjectDirCanonical", () => {
  ...
  it("a symlink and its target produce one canonical project identity", () => {
    const target = makeRepo("symlink-target");
    const link = join(tmpdir(), `ctx-symlink-${Date.now()}-${Math.random()}`);
    symlinkSync(target, link, "dir");
    cleanup.push(link);

    expect(hashProjectDirCanonical(link)).toBe(hashProjectDirCanonical(target));
  });
...
describe("resolveSessionDbPath", () => {
  ...
  it("a symlinked project selects the target session DB", () => {
    const target = makeRepo("symlink-store");
    const link = join(tmpdir(), `ctx-store-symlink-${Date.now()}-${Math.random()}`);
    symlinkSync(target, link, "dir");
    cleanup.push(link);
    const sessionsDir = makeSessionsDir();

    expect(resolveSessionDbPath({ projectDir: link, sessionsDir }))
      .toBe(resolveSessionDbPath({ projectDir: target, sessionsDir }));
  });
