 ...
 *
 * - Codex's plugin loader deserializes the root `.mcp.json` companion via
...
describe(".mcp.json", () => {
  ...
  it("does NOT use `${CODEX_PLUGIN_ROOT}` placeholders (no var expansion happens)", () => {
    const raw = readFileSync(resolve(REPO_ROOT, ".mcp.json"), "utf8");
...
describe(".codex-plugin/plugin.json", () => {
  ...
  it("references the root mcp.json via the camelCase `mcpServers` field", () => {
